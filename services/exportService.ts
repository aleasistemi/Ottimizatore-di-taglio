...
export const exportService = {
  toPdf: (results: OptimizationResult, cliente: string, commessa: string, groupBars: boolean = false) => {
    ...
    doc.text("OTTIMIZZATORE PROFESSIONALE DI TAGLIO", margin, 31);
    ...
    for (const [code, data] of Object.entries(results)) {
      ...
      doc.text(`${code} - ${data.descrizione}`, margin + 5, y + 8);
      ...
      barreToPrint.forEach((bar, bIdx) => {
        ...
        doc.text(`${label} - Taglio Totale: ${bar.somma}mm | Sfrido: ${bar.residuo}mm`, margin, y);
        
        y += 6;
        doc.setFont("helvetica", "normal");
        doc.setFontSize(10);
        doc.text(`Schema: ${bar.riepilogo}`, margin, y);
  ...
  panelToPdf: (results: PanelOptimizationResult, cliente: string, commessa: string, sheetW: number, sheetH: number) => {
    ...
    Object.values(results).forEach(group => {
      group.sheets.forEach((sheet, sIdx) => {
        ...
        doc.setFontSize(10);
        doc.setTextColor(220, 38, 38);
        doc.text("DISTINTA TAGLIO PANNELLI", margin, 26);
        ...
        doc.setTextColor(15, 23, 42);
        doc.setFontSize(13);
        doc.text(`Materiale: ${group.material}`, margin, 44);
        doc.setFontSize(11);
        doc.text(`Lastra Grezza: ${sheetW} x ${sheetH} mm | Foglio ${sIdx + 1}`, margin, 50);

        // Drawing the sheet layout
  ...
  toCsv: (results: OptimizationResult, groupBars: boolean = false) => {
    let csv = "Profilo;Quantità;Somma Tagli;Sfrido;Riepilogo\n";
    ...
    link.download = "ALEA_Export_Barre.csv";
    link.click();
  },

  panelsToCsv: (results: PanelOptimizationResult) => {
    let csv = "Materiale;Colore;Larghezza;Altezza;Rotazione\n";
    Object.values(results).forEach(g => {
      g.sheets.forEach(s => {
        s.panels.forEach(p => {
          csv += `${p.material};${p.colore || ''};${p.w};${p.h};${p.rotated}\n`;
        });
      });
    });
    const blob = new Blob([csv], { type: 'text/csv' });
    const link = document.createElement("a");
    link.href = URL.createObjectURL(blob);
    link.download = "ALEA_Export_Pannelli.csv";
    link.click();
  }
};
