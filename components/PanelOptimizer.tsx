...
import { Plus, Download, Trash2, Layout, FileText, Square, Save, Ruler, Palette, ChevronRight, Boxes } from 'lucide-react';
    ...
    <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 pb-10 animate-in fade-in duration-500">
      <div className="space-y-6">
        <section className="bg-white p-6 rounded-[2rem] border shadow-xl">
           <h3 className="text-xs font-black uppercase text-slate-400 mb-6 flex items-center gap-2"><FileText className="w-4 h-4" /> Intestazione</h3>
           <div className="space-y-4">
             <div>
               <label className="text-[10px] font-black text-slate-400 uppercase mb-1 block">Cliente</label>
               <input list="clients-list" type="text" value={cliente} onChange={e=>setCliente(e.target.value)} placeholder="Seleziona cliente..." className="w-full p-3 border rounded-xl font-bold outline-none focus:ring-2 focus:ring-red-500" />
               <datalist id="clients-list">{availableClients.map(c => <option key={c.id} value={c.nome} />)}</datalist>
             </div>
             <div>
               <label className="text-[10px] font-black text-slate-400 uppercase mb-1 block">Commessa</label>
               <input type="text" value={commessa} onChange={e=>setCommessa(e.target.value)} placeholder="ID commessa..." className="w-full p-3 border rounded-xl font-bold outline-none focus:ring-2 focus:ring-red-500" />
             </div>
           </div>
        </section>

        <section className="bg-white p-6 rounded-[2rem] border shadow-xl">
           <h3 className="text-xs font-black uppercase text-slate-400 mb-6 flex items-center gap-2"><Square className="w-4 h-4" /> Setup Lastra Grezza</h3>
           <select value={selectedPanelId} onChange={e=>handleSelectPanel(e.target.value)} className="w-full px-4 py-3 bg-slate-900 text-white rounded-xl font-black text-xs uppercase mb-4 outline-none">
              <option value="">-- Seleziona da Archivio --</option>
           ...
           <div className="space-y-4">
             ...
             <div className="grid grid-cols-2 gap-4">
                <div className="space-y-1">
                   <label className="text-[9px] font-black uppercase text-slate-400">Materiale (Nome/Spess.)</label>
        ...
        <section className="bg-white p-6 rounded-[2rem] border shadow-xl">
           <h3 className="text-xs font-black uppercase text-slate-400 mb-6 flex items-center gap-2"><Plus className="w-4 h-4" /> Inserimento Pezzo</h3>
           ...
           <button onClick={handleAddCut} className="w-full bg-red-600 text-white font-black py-4 rounded-2xl shadow-lg hover:bg-red-700 transition-all active:scale-95">AGGIUNGI PEZZO</button>
        </section>
      </div>
      ...
      <div className="lg:col-span-2 space-y-6">
        <section className="bg-white rounded-[2.5rem] border shadow-xl flex flex-col h-[500px]">
          <div className="p-6 border-b bg-slate-50 flex justify-between items-center">
            <h3 className="text-xs font-black uppercase text-slate-500">Distinta Pezzi Multi-Materiale</h3>
            <div className="flex gap-4">
              <button onClick={saveCommessa} className="text-[10px] font-black bg-blue-600 text-white px-4 py-1.5 rounded-full flex gap-1 items-center hover:bg-blue-700"><Save className="w-3 h-3" /> Archivia</button>
          ...
          <div className="flex-1 overflow-y-auto">
             <table className="w-full text-left text-xs">
                ...
                <tbody>
                  {distinta.map(c=>(
                    <tr key={c.id} className="border-b font-bold group hover:bg-slate-50">
                       <td className="p-4">
                         ...
                         <div className="text-[10px] text-red-600 font-black uppercase">{c.colore}</div>
                       </td>
                  ...
                  {distinta.length === 0 && (
                    <tr><td colSpan={4} className="p-20 text-center text-slate-300 italic font-medium">Nessun pezzo inserito.</td></tr>
                  )}
                </tbody>
             </table>
          </div>
          <div className="p-6 bg-slate-50 border-t"><button onClick={runOptimization} className="w-full bg-slate-900 text-white py-5 rounded-2xl font-black uppercase shadow-2xl hover:bg-slate-800 transition-all">{isOptimizing ? "Ottimizzazione..." : "Calcola Nesting Professionale"}</button></div>
        </section>
        ...
        {results && (
          <div className="space-y-10 animate-in slide-in-from-bottom-5 duration-500">
             <div className="flex justify-between items-center bg-white p-6 rounded-[2rem] border shadow-lg">
                ...
                <button onClick={()=>exportService.panelToPdf(results, cliente, commessa, parseFloat(larghezzaLastra), parseFloat(altezzaLastra), coloreLastra, "N/D")} className="bg-red-600 text-white px-8 py-3 rounded-xl font-black flex gap-2 shadow-lg hover:bg-red-700 transition-all"><Download /> PDF PROFESSIONALE</button>
             </div>
                ...
                <div key={key} className="space-y-6">
                   <div className="bg-slate-900 p-6 rounded-3xl text-white flex justify-between items-center shadow-xl">
                      <div>
                        ...
                        <p className="text-xs font-bold text-slate-400 uppercase">{key.split('___')[2] || 'Standard'}</p>
                      </div>
                      <div className="text-right">
                        <div className="text-3xl font-black">{group.sheets.length}</div>
                        <div className="text-[10px] font-black text-slate-400 uppercase">Lastre {group.material}</div>
                      </div>
                   </div>
                   ...
                   {group.sheets.map((sheet: any, sIdx: number) => (
                      <div key={sIdx} className="bg-white p-8 rounded-[2.5rem] border shadow-xl space-y-6">
                         <div className="flex justify-between items-center border-b pb-4">
                            <span className="text-sm font-black text-slate-800 flex items-center gap-2 uppercase tracking-tighter"><Boxes className="w-4 h-4 text-red-600" /> FOGLIO {sIdx + 1}</span>
                            <span className="text-[10px] font-black text-slate-400 uppercase bg-slate-100 px-4 py-1.5 rounded-full">Sfrido: {sheet.residuo.toLocaleString()} mm²</span>
                         </div>
                         <div className="flex justify-center bg-slate-50 p-6 rounded-3xl border border-dashed border-slate-200">
                            <canvas 
                              ...
                              className="max-w-full rounded-lg shadow-2xl bg-white border" 
                            />
                         </div>
                      </div>
                   ))}
