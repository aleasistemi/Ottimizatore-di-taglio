...
import { Database, Plus, Search, Trash2, Edit3, X, Users, Briefcase, Globe, Settings, CheckCircle2, ShieldCheck, Copy, Square, CloudUpload, CloudDownload, RefreshCw, LogOut, AlertCircle, Info, ExternalLink, Calendar, Eye, Activity, Ruler, Maximize } from 'lucide-react';
...
export const ProfileDatabase: React.FC<ProfileDatabaseProps> = ({ onOpenCommessa, forcedTab, onTabChange }) => {
  ...
  const handleSavePanelMaterial = async () => {
    ...
    setIsAdding(false); setPanelForm({ id: '', codice: '', descr: '', materiale: 'Lexan', spessori: '2, 3, 4', lungDefault: 3050, altDefault: 2050 });
  };
                ...
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-10">
                   <div className="bg-slate-900 p-8 rounded-[3.5rem] text-white shadow-2xl space-y-8">
                      <div className="space-y-4">
                        ...
                        <a href="https://supabase.com" target="_blank" className="flex items-center justify-center gap-3 w-full bg-blue-600 hover:bg-blue-500 text-white font-black py-5 rounded-2xl text-xs">SUPABASE.COM</a>
                      </div>
                      <div className="space-y-4 pt-4 border-t border-slate-800">
                        ...
                        <input type="text" value={sbUrl} onChange={e=>setSbUrl(e.target.value)} placeholder="URL Progetto..." className="w-full px-5 py-4 rounded-xl bg-slate-800 border border-slate-700 font-mono text-xs text-blue-300" />
                        <input type="password" value={sbKey} onChange={e=>setSbKey(e.target.value)} placeholder="Anon Key..." className="w-full px-5 py-4 rounded-xl bg-slate-800 border border-slate-700 font-mono text-xs text-blue-300" />
                        <div className="grid grid-cols-2 gap-3">
                           <button onClick={handleConnectSupabase} className="bg-blue-600 text-white font-black py-4 rounded-2xl">COLLEGA</button>
                           <button onClick={() => { supabaseService.disconnect(); setIsConnected(false); }} className="bg-red-600 text-white font-black py-4 rounded-2xl">SCOLLEGA</button>
                        </div>
                      </div>
                   </div>
                ...
                <div className="p-8 rounded-[2.5rem] border-2 border-dashed border-red-200 bg-slate-50/50 animate-in zoom-in-95 duration-200">
                   ...
                   {activeTab === 'pannelli' && (
                     <div className="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-5 gap-4">
                        ...
                        <div><label className="text-[10px] font-black uppercase text-slate-400 mb-1 block">Base (mm)</label><input type="number" value={panelForm.lungDefault} onChange={e=>setPanelForm({...panelForm, lungDefault: parseInt(e.target.value)})} className="w-full px-4 py-3 rounded-xl border border-slate-200 font-bold" /></div>
                        <div><label className="text-[10px] font-black uppercase text-slate-400 mb-1 block">Altezza (mm)</label><input type="number" value={panelForm.altDefault} onChange={e=>setPanelForm({...panelForm, altDefault: parseInt(e.target.value)})} className="w-full px-4 py-3 rounded-xl border border-slate-200 font-bold" /></div>
                    ...
                    <tbody className="divide-y divide-slate-50">
                       ...
                       {activeTab === 'pannelli' && panelMaterials.filter(p=>p.codice.includes(searchTerm.toUpperCase()) || p.materiale.includes(searchTerm)).map(p => (
                          <tr key={p.id} className="hover:bg-slate-50/80 transition-all font-bold group">
                             <td className="px-8 py-6 text-xs font-black text-slate-900">{p.codice}</td>
                             <td className="px-8 py-6 text-sm text-blue-600">{p.materiale} {p.spessori}mm</td>
