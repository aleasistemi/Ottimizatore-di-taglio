...
-- ESEGUI QUESTO SCRIPT PER RIPARARE LE TABELLE SENZA PERDERE DATI

-- 1. Ripara Tabella Profili (se mancano colonne)
ALTER TABLE profiles ADD COLUMN IF NOT EXISTS "lungMax" INTEGER;

-- 2. Ripara Tabella Pannelli (Rimuove i duplicati e aggiunge i mancanti)
-- ATTENZIONE: Questo script NON cancella i tuoi dati.
ALTER TABLE panel_materials ADD COLUMN IF NOT EXISTS "lungDefault" INTEGER;
ALTER TABLE panel_materials ADD COLUMN IF NOT EXISTS "altDefault" INTEGER;
ALTER TABLE panel_materials ADD COLUMN IF NOT EXISTS "giraPezzoDefault" BOOLEAN DEFAULT true;

-- Opzionale: Se vuoi pulire le colonne minuscole inutilizzate dopo la riparazione:
-- ALTER TABLE panel_materials DROP COLUMN IF EXISTS lungdefault;
-- ALTER TABLE panel_materials DROP COLUMN IF EXISTS altdefault;
  `.trim();
        ...
        {activeTab === 'settings' ? (
           <div className="max-w-3xl mx-auto space-y-10">
              <div className="text-center space-y-2">
                ...
                <p className="text-slate-400 text-sm">Collega Supabase per non perdere mai i tuoi dati.</p>
              </div>
              ...
              <div className="bg-slate-50 p-8 rounded-[2rem] border-2 border-slate-100 space-y-4">
                ...
                <button onClick={() => window.location.reload()} className="w-full bg-slate-900 text-white font-black py-4 rounded-2xl">Connetti e Sincronizza</button>
              </div>

              <div className="space-y-4">
                <div className="flex items-center gap-2 text-blue-600 font-black text-xs uppercase"><Code className="w-5 h-5" /> Riparazione Database (SQL Editor)</div>
                <div className="bg-slate-900 p-6 rounded-[2rem] text-white relative">
                  <button onClick={() => navigator.clipboard.writeText(sqlRepairScript)} className="absolute top-4 right-4 bg-white/10 p-2 rounded-xl"><Copy className="w-4 h-4" /></button>
                  <pre className="text-[10px] font-mono leading-tight text-blue-300 overflow-x-auto">{sqlRepairScript}</pre>
                </div>
              </div>
           </div>
        ) : (
           <div className="space-y-6">
              ...
              {isAdding && activeTab === 'pannelli' && (
                <div className="p-6 bg-slate-50 border rounded-[2rem] animate-in zoom-in-95 space-y-4">
                   ...
                   <button onClick={handleSavePanel} className="w-full bg-slate-900 text-white py-4 rounded-xl font-black uppercase">Salva Pannello</button>
                </div>
              )}
