...
export const BarOptimizer: React.FC<BarOptimizerProps> = ({ externalData }) => {
  ...
  useEffect(() => {
    ...
    // Caricamento dati esterni se presenti
  ...
  const handleAddCut = () => {
    ...
    const angoli = `${angoloSx}/${angoloDx}`;
  ...
  const removeCut = (id: string) => {
    setDistinta(prev => prev.filter(c => c.id !== id));
  };
  ...
  const getGroupedBars = (barre: OptimizedBar[]): GroupedBarResult[] => {
    ...
    barre.forEach(bar => {
      ...
      if (groups[fingerprint]) {
        ...
        groups[fingerprint] = { ...bar, count: 1 };
      }
    });
    ...
    barre.forEach(bar => {
      bar.tagli.forEach(t => {
        ...
        if (summary[key]) {
          ...
          summary[key] = { qty: 1, lung: t.lung, angoli: t.angoli };
        }
      });
    });
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-1 space-y-6">
            ...
            <div className="space-y-5">
              <div>
                ...
                <datalist id="clients-list">
                  {availableClients.map(c => <option key={c.id} value={c.nome} />)}
                </datalist>
              </div>
              <div>
                ...
                <input 
                  ...
                  className="w-full px-4 py-3 bg-slate-50 border border-slate-200 rounded-xl focus:ring-2 focus:ring-red-500 outline-none transition-all font-medium"
                />
              </div>
            </div>
          </section>
          ...
          <section className="bg-white p-6 rounded-3xl border border-slate-200 shadow-xl">
            ...
            <div className="space-y-4">
              <div>
                ...
                >
                  ...
                  {availableProfiles.map(p => (
                    <option key={p.codice} value={p.codice}>{p.codice} - {p.descr}</option>
                  ))}
                </select>
              </div>
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                <div>
                   <div className="flex justify-between items-center mb-1">
                      ...
                      <button onClick={resetAngles} title="Reset 90/90" className="text-[9px] font-black text-red-500 hover:text-red-700 flex items-center gap-0.5 transition-colors uppercase">
              ...
              <button 
                ...
                className="w-full bg-red-600 hover:bg-red-700 text-white font-black py-4 rounded-2xl shadow-xl transition-all active:scale-95 flex items-center justify-center space-x-3 mt-4"
              >
          ...
          <section className="bg-slate-900 p-6 rounded-3xl text-white shadow-xl">
             <h3 className="text-xs font-black mb-4 flex items-center space-x-2 uppercase tracking-widest text-slate-400">
               <Settings className="w-4 h-4" />
               <span>Setup Tecnico</span>
             </h3>
             <div className="space-y-4">
               <div className="grid grid-cols-2 gap-4">
                  <div>
          ...
          <section className="bg-white rounded-3xl border border-slate-200 shadow-xl overflow-hidden flex flex-col h-[400px]">
                  ...
                  {distinta.length === 0 ? (
                    <tr>
                      <td colSpan={4} className="px-6 py-20 text-center text-slate-300 italic">
                        Inserisci i tagli per iniziare.
                      </td>
                    </tr>
                  ) : (
                    distinta.map((cut) => (
                      <tr key={cut.id} className="hover:bg-slate-50/80 font-bold">
                        ...
                        <td className="px-6 py-4 text-center">
                          <button onClick={() => removeCut(cut.id)} className="p-2 text-slate-300 hover:text-red-600"><Trash2 className="w-4 h-4" /></button>
                        </td>
                      </tr>
                    ))
                  )}
            ...
            <div className="p-5 bg-slate-50 border-t">
              <button 
                ...
                disabled={distinta.length === 0 || isOptimizing}
                className="w-full bg-slate-900 hover:bg-slate-800 disabled:bg-slate-300 text-white font-black py-4 rounded-2xl transition-all flex items-center justify-center space-x-3 shadow-2xl"
              >
            ...
            <div className="space-y-6 animate-in fade-in slide-in-from-bottom-8 duration-700">
               <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                  ...
                  <div className="flex gap-2">
                    <button onClick={handleExportCsv} className="flex items-center justify-center space-x-2 bg-white border-2 border-slate-200 hover:border-slate-800 px-5 py-2.5 rounded-xl text-xs font-black text-slate-700 shadow-sm transition-all">
                      <FileSpreadsheet className="w-4 h-4 text-green-600" />
                      <span>CSV</span>
                    </button>
                    <button onClick={handleExportPdf} className="flex items-center justify-center space-x-2 bg-white border-2 border-slate-200 hover:border-red-500 px-5 py-2.5 rounded-xl text-xs font-black text-slate-700 shadow-sm transition-all">
                   ...
                   <div key={code} className="space-y-4">
                     ...
                     <div className="bg-white rounded-[2rem] border border-slate-200 shadow-lg overflow-hidden">
                        <div className="px-6 py-4 bg-slate-50 border-b flex items-center gap-2">
                           ...
                           <h5 className="text-[10px] font-black uppercase text-slate-600 tracking-widest">Riepilogo Tagli Univoci</h5>
                        </div>
                     ...
                     <div className="bg-white rounded-[2rem] border border-slate-200 shadow-2xl overflow-hidden">
                       <div className="p-6 bg-slate-900 text-white flex justify-between items-center">
                          <div className="space-y-1">
                            <h4 className="font-black text-2xl tracking-tighter text-red-500">{code}</h4>
                            <p className="text-[10px] text-slate-400 font-bold uppercase tracking-widest">{data.descrizione}</p>
                          </div>
                       </div>
                           ...
                           <div key={idx} className="space-y-4 group">
                             <div className="flex items-end justify-between border-b border-slate-100 pb-2">
                               <span className="text-sm font-black text-slate-800 uppercase">
                                  {bar.count > 1 ? `${bar.count}x BARRE IDENTICHE` : `BARRA ${idx + 1}`}
                               </span>
                               <div className="text-[10px] font-bold text-slate-400 space-x-3 italic">
                                  <span>Taglio: <b className="text-slate-700">{bar.somma} mm</b></span>
                                  <span>Scarto: <b className="text-red-500">{bar.residuo} mm</b></span>
                               </div>
                             </div>
                             
                             <div className="h-10 w-full bg-slate-100 rounded-xl overflow-hidden flex border-2 border-slate-200/50 shadow-inner">
                               {bar.tagli.map((t, tIdx) => (
                                 <div 
                                   key={tIdx} 
                                   className="h-full border-r-2 border-white/40 flex items-center justify-center text-[10px] text-white font-black overflow-hidden"
                                   style={{ 
                                     width: `${(t.lung / (bar.somma + bar.residuo)) * 100}%`,
                                     backgroundColor: `hsl(${350 - (tIdx * 15)}, 80%, 55%)`
                                   }}
                                 >
                                   <span className="truncate px-1">{t.lung}</span>
                                 </div>
                               ))}
                               <div className="h-full bg-slate-300/20 flex items-center justify-center text-[9px] text-slate-400 font-black italic" style={{ width: `${(bar.residuo / (bar.somma + bar.residuo)) * 100}%` }}>
                                 RES.
                               </div>
                             </div>

                             <div className="flex flex-wrap gap-2 text-[11px] font-bold text-slate-600 bg-slate-50 p-4 rounded-2xl border border-slate-100">
                                <span className="text-red-600 font-black flex items-center gap-1 uppercase tracking-tighter">
                                  <ChevronRight className="w-3 h-3" /> SCHEMA:
                                </span>
                                {bar.riepilogo}
                             </div>
                           </div>
